...
module.exports = class Message {
  ...
  async sendSingleActionCard(actionCard, opts) {
    assert(actionCard, 'actionCard required');
    assert(actionCard.title, 'actionCard.title required');
    assert(actionCard.text, 'actionCard.text required');
    assert(actionCard.singleTitle, 'actionCard.singleTitle required');
    assert(actionCard.singleURL, 'actionCard.singleURL required');

    return await this.send(
      Object.assign({ msgtype: 'actionCard', actionCard }, opts)
    );
  }

  async sendBtnsActionCard(actionCard, opts) {
    assert(actionCard, 'actionCard required');
    assert(actionCard.title, 'actionCard.title required');
    assert(actionCard.text, 'actionCard.text required');
    assert(actionCard.btns, 'actionCard.btns required');
  ...
  async sendFeedCard(feedCard, opts) {
    assert(feedCard, 'feedCard required');

    return await this.send(
      Object.assign({ msgtype: 'feedCard', feedCard }, opts)
    );
  }
